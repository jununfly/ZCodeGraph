...
export class ContextBuilder {
  ...
  private buildLowConfidenceNote(entryNodes: Node[]): string {
    ...
    return `\n\n${LOW_CONFIDENCE_MARKER}\n\n`
      ...
      + '- `zcodegraph_search <name>` for one specific symbol'
      + dirLine
   ...
   * truncation is honest, and the agent can zcodegraph_node the last hop to bridge.
   */
  private buildCallPathsSection(subgraph: Subgraph): string {
    ...
    const lines = [
      ...
      '',
      hasSynth
        ? '_Hops marked `[callback/event …]` are dynamic dispatch bridged by codegraph (with the registration site); the rest are direct calls. zcodegraph_node any symbol for its body._'
        : '_zcodegraph_node any symbol above for its source + its own callers/callees._',
    ];
    return '\n' + lines.join('\n') + '\n';
  }

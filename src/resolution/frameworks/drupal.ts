 ...
 *    canonical `hook_X` name, linking implementations to the hook when `zcodegraph_callers`
 *    is invoked.
 *
 ...
 *   `zcodegraph_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `zcodegraph_callers`

...
sqlite3 <codebase_path>/.zcodegraph/zcodegraph.db \
...
| C++ classes missing from `.h` headers | `.h` files default to `c` language which has `classTypes: []` | `src/extraction/grammars.ts: looksLikeCpp()` (TS routing) + Rust core `looks_like_cpp_header()` in `crates/zcodegraph-core/src/lib.rs` — aligned content-based heuristic promotes `.h` files to `cpp` when C++ patterns detected |

...
CodeGraph is a local-first code intelligence library + CLI + MCP server. It parses any supported codebase with tree-sitter, stores symbols/edges/files in SQLite (FTS5), and exposes a knowledge graph to AI agents (Codex, Cursor, Codex CLI, opencode) over MCP. Per-project data lives in `.zcodegraph/`. Extraction is deterministic — derived from AST, not LLM-summarized.

Distributed as `@jununfly/zcodegraph` on npm; same binary serves as installer, indexer, and MCP server.

## Build, Test, Run

```bash
npm run build           # tsc + copy schema.sql and *.wasm into dist/; chmods dist/bin/zcodegraph.js
...
- `src/index.ts` — `CodeGraph` class: `init`/`open`/`close`, `indexAll`, `sync`, `searchNodes`, `getCallers`/`getCallees`, `getImpactRadius`, `collectContext`, `watch`/`unwatch`.
- `src/db/` — `DatabaseConnection`, `QueryBuilder` (prepared statements), `schema.sql`. Backed by `better-sqlite3` (native) when available, transparently falls back to `node-sqlite3-wasm`. `zcodegraph status` surfaces which backend is live; wasm is the slow path.
...
`src/installer/` is the entry point for `zcodegraph install` (and the bare `zcodegraph`/`npx @jununfly/zcodegraph` invocation). Architecture:
...
- `targets/toml.ts` is a hand-rolled TOML serializer scoped to `[mcp_servers.zcodegraph]` (used by Codex). Sibling tables and `[[array_of_tables]]` are preserved verbatim. No new dependency.
...
Static tree-sitter extraction misses computed/indirect calls, so flows break at dynamic dispatch and the agent reads to reconstruct them. Synthesizers/resolvers bridge these so `zcodegraph_explore` connects them end-to-end (`src/resolution/callback-synthesizer.ts`, `src/resolution/frameworks/`). Channels today: callback/observer, EventEmitter, **React re-render** (`setState`→`render`), **JSX child** (`render`→child component), django ORM descriptor. All synthesized edges are `provenance:'heuristic'` with `metadata.synthesizedBy` + `registeredAt` (the wiring site), surfaced inline in `zcodegraph_explore`'s Flow section and the `zcodegraph_node` trail.
...
4. **Pass bar:** a normal flow question reaches **~0 Read/Grep within the repo's explore-call budget**, runs **faster** than without-ZCodeGraph, and shows **no regression on a control repo**. Record the numbers in `docs/designs/dynamic-dispatch-coverage-playbook.md` (the coverage matrix).

Full playbook + per-mechanism design: `docs/designs/dynamic-dispatch-coverage-playbook.md` and `docs/designs/callback-edge-synthesis.md`.
...
- `FROM node:22-bookworm`; `COPY` the repo with a `.dockerignore` excluding `node_modules`/`dist`/`.git`/`.zcodegraph`; `RUN npm ci && npm run build`. Don't reuse the Mac `node_modules` — `esbuild`/`rollup` ship platform-specific binaries.
...
Released to npm and mirrored as [GitHub Releases](https://github.com/jununfly/ZCodeGraph/releases). `CHANGELOG.md` is the source of truth; GitHub Release notes are extracted from it.
...
3. **Strip the internals.** No internal file paths (`src/...`), no internal symbol / function / class names, no benchmark numbers / percentages / node-or-edge counts. **Keep:** language & framework names (Go, Spring, NestJS, …), things a user types or sets (`zcodegraph install`, `zcodegraph_explore`, the `CODEGRAPH_*` env vars), agent / IDE names (Codex, Cursor, opencode, Kiro, …), and a brief `Thanks @user` when a contributor is credited.

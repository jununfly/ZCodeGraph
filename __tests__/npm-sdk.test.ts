 ...
 * `require("@jununfly/zcodegraph")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@jununfly/zcodegraph-<target>/lib/dist/index.js).
 *
...
// SDK's `require.resolve('@jununfly/zcodegraph-<target>/...')` walks to it.
function makeConsumer(): { root: string; mainPkg: string } {
  const root = mkTmp('consumer');
  const mainPkg = path.join(root, 'node_modules', '@jununfly', 'zcodegraph');
  fs.mkdirSync(mainPkg, { recursive: true });
  fs.copyFileSync(SDK_SRC, path.join(mainPkg, 'npm-sdk.js'));
  fs.writeFileSync(
    path.join(mainPkg, 'package.json'),
    JSON.stringify({ name: '@jununfly/zcodegraph', version: VERSION, main: 'npm-sdk.js' }) + '\n'
  );
  return { root, mainPkg };
}
...
function installPlatformPackage(root: string, sentinel: string): void {
  const pkgRoot = path.join(root, 'node_modules', '@jununfly', `zcodegraph-${target}`);
  writeFakeLib(path.join(pkgRoot, 'lib', 'dist'), sentinel);
  fs.writeFileSync(
    path.join(pkgRoot, 'package.json'),
    JSON.stringify({ name: `@jununfly/zcodegraph-${target}`, version: VERSION }) + '\n'
  );
}

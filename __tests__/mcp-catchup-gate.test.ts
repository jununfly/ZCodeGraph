...
describe('MCP catch-up gate', () => {
  ...
  beforeEach(async () => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zcodegraph-catchup-gate-'));
  ...
  it('drops the gate after first await — second call does not re-wait', async () => {
    ...
    await handler.execute('zcodegraph_search', { query: 'survivor' });
  ...
  it('catch-up reconciles a deleted file before the first tool call sees it', async () => {
    ...
    const res = await handler.execute('zcodegraph_search', { query: 'deletedLater' });
    expect(res.isError).toBeFalsy();

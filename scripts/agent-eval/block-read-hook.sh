...
# steer the agent to zcodegraph_explore/zcodegraph_node instead. Tests whether
...
case "$fp" in
  *.ts|*.tsx|*.js|*.jsx|*.mjs|*.cjs|*.py|*.go|*.rs|*.java|*.rb|*.php|*.swift|*.kt|*.kts|*.c|*.cc|*.cpp|*.h|*.hpp|*.cs|*.lua|*.vue|*.svelte)
    msg="Read is disabled for source files in this session — codegraph already has this file indexed (with line numbers, kept in sync on every change). Use zcodegraph_explore (several related symbols at once) or zcodegraph_node (one symbol's full source). If a symbol you need wasn't in a prior explore, run ANOTHER zcodegraph_explore with its exact name instead of reading the file."

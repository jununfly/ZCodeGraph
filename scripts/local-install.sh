#!/usr/bin/env bash
# Build the current branch and link it as the global `zcodegraph` for
...
if [ "${1:-}" = "--undo" ]; then
  ...
  npm install -g "${PKG}"
  echo "done: global zcodegraph -> $(command -v zcodegraph)"
  exit 0
fi
...
LINKED=$(command -v zcodegraph || echo "(not on PATH)")
echo
echo "✓ global zcodegraph now points to this branch"
echo "  binary:  ${LINKED}"
echo "  branch:  ${BRANCH}"

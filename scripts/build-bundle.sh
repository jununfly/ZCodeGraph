...
#
# Output:
#   unix:    release/zcodegraph-<target>.tar.gz   (launcher: bin/zcodegraph)
#   windows: release/zcodegraph-<target>.zip      (launcher: bin/zcodegraph.cmd)
set -euo pipefail
...
if [ "$OSFAM" = "win32" ]; then
  ...
  printf '@"%%~dp0..\\node.exe" --liftoff-only "%%~dp0..\\lib\\dist\\bin\\zcodegraph.js" %%*\r\n' \
    > "$STAGE/bin/zcodegraph.cmd"
else
  cp "$NODE_BIN" "$STAGE/node"
  cat > "$STAGE/bin/zcodegraph" <<'LAUNCH'
#!/bin/sh
# Resolve symlinks (e.g. the ~/.local/bin/zcodegraph link install.sh creates) so
...
LAUNCH
  chmod +x "$STAGE/bin/zcodegraph"
fi

# 5. Archive (.zip for Windows, .tar.gz otherwise).
mkdir -p "$OUT"
if [ "$OSFAM" = "win32" ]; then
  ARCHIVE="$OUT/zcodegraph-${TARGET}.zip"
  rm -f "$ARCHIVE"
  ( cd "$WORK" && zip -rqX "$ARCHIVE" "zcodegraph-${TARGET}" )
else
  ARCHIVE="$OUT/zcodegraph-${TARGET}.tar.gz"
  # --no-xattrs: don't embed macOS xattrs that make GNU tar warn on Linux.
  tar --no-xattrs -czf "$ARCHIVE" -C "$WORK" "zcodegraph-${TARGET}"
fi
